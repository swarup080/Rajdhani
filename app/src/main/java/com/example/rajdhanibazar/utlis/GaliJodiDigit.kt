...
import com.example.rajdhanibazar.databinding.ActivityGaliJodiDigitBinding
import com.example.rajdhanibazar.databinding.ActivityJodiDigitBinding
...
class GaliJodiDigit : AppCompatActivity() {
    private lateinit var binding: ActivityGaliJodiDigitBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityGaliJodiDigitBinding.inflate(layoutInflater)
    ...
    private fun getDigit() {
        ...
        if (gameOpenDigit.length > 2) {
            Toast.makeText(this, "Digit should be 11-99", Toast.LENGTH_SHORT).show()
        } else {
            ...
            RetrofitInstance.apiInterface.postBid(addBid).enqueue(object : Callback<BidResponse?> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        val navigate = Intent(this@GaliJodiDigit, MainActivity::class.java)
                        navigate.addFlags(
                        ...
                        Toast.makeText(this@GaliJodiDigit, bidResponse?.message, Toast.LENGTH_SHORT)
                            .show()
                    } else {
                        // Handle failure in bid creation
                        Toast.makeText(this@GaliJodiDigit, "Failed to create bid", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
                ...
                override fun onFailure(call: Call<BidResponse?>, t: Throwable) {
                    ...
                    Toast.makeText(this@GaliJodiDigit, t.message, Toast.LENGTH_SHORT).show()
                }
            })
        } else {
            // Show a message if the balance is insufficient
            Toast.makeText(this@GaliJodiDigit, "Insufficient balance", Toast.LENGTH_SHORT).show()
        }
    }
            ...
            .enqueue(object : Callback<ResponseBody> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        Toast.makeText(this@GaliJodiDigit, responseBody, Toast.LENGTH_SHORT).show()
                    } else {
                        Toast.makeText(
                            this@GaliJodiDigit,
