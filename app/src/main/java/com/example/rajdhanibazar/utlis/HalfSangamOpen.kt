...
import com.example.rajdhanibazar.databinding.ActivityHalfSangamOpenBinding
...
class HalfSangamOpen : AppCompatActivity() {
    private lateinit var binding: ActivityHalfSangamOpenBinding
    ...
    private var openDigit: String? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityHalfSangamOpenBinding.inflate(layoutInflater)
        ...
        Log.d("HalfSangamOpen", "onCreate called")
    ...
    private fun setupAddBidButtonListener() {
        binding.addBid.setOnClickListener {
            ...
            if (openDigit != null && closePanna != null && enterBalance >= 10) {
                sendBidRequest()
            } else {
                Log.d("HalfSangamOpen", "Validation failed: OpenDigit, ClosePanna, or EnterBalance is incorrect.")
                Toast.makeText(this, "Invalid bid. Check Open Digit, Close Panna, and Balance.", Toast.LENGTH_SHORT).show()
            }
        }
    }
                ...
                .enqueue(object : Callback<HalfSangamResponse> {
                    override fun onResponse(call: Call<HalfSangamResponse>, response: Response<HalfSangamResponse>) {
                        if (response.isSuccessful) {
                            Toast.makeText(this@HalfSangamOpen, response.body()?.message ?: "Success", Toast.LENGTH_SHORT).show()
                            ...
                            Toast.makeText(this@HalfSangamOpen, "Failed to store Half Sangam", Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onFailure(call: Call<HalfSangamResponse>, t: Throwable) {
                        Log.e("HalfSangamOpen", "API Request Failed: ${t.message}")
                        Toast.makeText(this@HalfSangamOpen, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                })
        }
    }
    ...
    private fun getPanna() {
        val gameOpenDigit = binding.openPanna.text.toString().trim()
        val gameClosePanna = binding.closePanna.text.toString().trim()

        if (gameOpenDigit.length != 1 || gameClosePanna.length != 3) {
        ...
        try {
            ...
            if (digit in 0..9) {
                openDigit = gameOpenDigit
            } else {
                Toast.makeText(this, "Open Digit out of range (0-9)", Toast.LENGTH_SHORT).show()
                return
            }
            ...
            if (digit2 in 111..999) {
                ...
                Toast.makeText(this, "Close Panna out of range (111-999)", Toast.LENGTH_SHORT).show()
                return
            }
            ...
            .enqueue(object : Callback<ResponseBody> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        Toast.makeText(this@HalfSangamOpen, responseBody, Toast.LENGTH_SHORT).show()
                    } else {
                        Toast.makeText(
                            this@HalfSangamOpen,
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    Toast.makeText(this@HalfSangamOpen, t.message, Toast.LENGTH_SHORT).show()
                }
            })
    }

    private fun navigateToMainActivity() {
        val intent = Intent(this@HalfSangamOpen, MainActivity::class.java)
