...
import com.example.rajdhanibazar.databinding.ActivityStarlineSinglePannaBinding
...
class StarlineSinglePanna : AppCompatActivity() {
    private lateinit var binding: ActivityStarlineSinglePannaBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivityStarlineSinglePannaBinding.inflate(layoutInflater)
            ...
            RetrofitInstance.apiInterface.postBid(addBid).enqueue(object : Callback<BidResponse?> {
                override fun onResponse(p0: Call<BidResponse?>, p1: Response<BidResponse?>) {
                    if (p1.isSuccessful) {
                        ...
                        updateUserBalance()
                        val navigate = Intent(this@StarlineSinglePanna, MainActivity::class.java)
                        navigate.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK)
                        startActivity(navigate)
                        finish()
                        Toast.makeText(this@StarlineSinglePanna, bidResponse?.message, Toast.LENGTH_SHORT)
                            .show()
                    } else {
                        Toast.makeText(this@StarlineSinglePanna, "Failed to create bid", Toast.LENGTH_SHORT)
                            .show()
                    }
                }

                override fun onFailure(p0: Call<BidResponse?>, p1: Throwable) {
                    Toast.makeText(this@StarlineSinglePanna, p1.message, Toast.LENGTH_SHORT).show()
                }
            })
        } else {
            // Show a message if the balance is insufficient
            Toast.makeText(this@StarlineSinglePanna, "Insufficient balance", Toast.LENGTH_SHORT).show()
        }
    }
            ...
            .enqueue(object : Callback<ResponseBody> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        Toast.makeText(this@StarlineSinglePanna, responseBody, Toast.LENGTH_SHORT).show()
                    } else {
                        Toast.makeText(
                            this@StarlineSinglePanna,
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    Toast.makeText(this@StarlineSinglePanna, t.message, Toast.LENGTH_SHORT).show()
                }
            })
    }
}